...
static unsigned long base_qa7_address;
static unsigned int interval;

void systimer_init(unsigned long base_virt_address, unsigned long systimer_interval) {
    base_qa7_address = base_virt_address;
    interval = systimer_interval;

    // 今のカウンタ値から interval tick 後に発火するように2個目のレジスタに値をセットする
    unsigned int next = get32(base_qa7_address + SYSTIMER_CLO_OFFSET) + interval;
    put32(base_qa7_address + SYSTIMER_C1_OFFSET, next);
}
...
unsigned long get_physical_systimer_count() {
    unsigned long chi = get32(base_qa7_address + SYSTIMER_CHI_OFFSET);
    unsigned long clo = get32(base_qa7_address + SYSTIMER_CLO_OFFSET);
    if (chi != get32(base_qa7_address + SYSTIMER_CHI_OFFSET)) {
        chi = get32(base_qa7_address + SYSTIMER_CHI_OFFSET);
        clo = get32(base_qa7_address + SYSTIMER_CLO_OFFSET);
    }
    return clo | (chi << 32);
}
