...
// todo: まだ init を呼んでない
void init_interrupt_controller(unsigned long base_virt_address) {
...
void enable_interrupt_controller(unsigned long cpuid) {
    ...
    put32(base_address + GICC_PMR_OFFSET, 0xff);
    // CPU interface を有効化
    put32(base_address + GICC_CTLR_OFFSET, GICC_CTLR_ENABLE);
    ...
    put32(base_address + GICD_ISENABLER_OFFSET, (1 << IRQ_HYP_PHYS_TIMER) | (1 << IRQ_VIRT_TIMER));
}

// システムで1回実行する
void enable_legacy_interrupt_controller(void) {
    // GIC Distributor を有効化
    put32(base_address + GICD_CTLR_OFFSET, GICD_CTLR_ENABLE);
    ...
    unsigned int target_reg = get32(base_address + GICD_ITARGETSR_OFFSET + 32);
    target_reg |= (0x01 << 8);
    put32(base_address + GICD_ITARGETSR_OFFSET + 32, target_reg);

    // UART 割込みを有効化
    // SPI 1 の割込み番号は IRQ_UART0(33) で、ISENABLER[1] に相当する
    put32(base_address + GICD_ISENABLER_OFFSET + 4, (1 << (IRQ_UART0 - 32)));
}

// todo: これはドライバの API として用意する
void enable_virtual_timer_irq(void) {
    put32(base_address + GICD_ISENABLER_OFFSET, (1 << IRQ_VIRT_TIMER));
}

void disable_virtual_timer_irq(void) {
    put32(base_address + GICD_ICENABLER_OFFSET, (1 << IRQ_VIRT_TIMER));
}
