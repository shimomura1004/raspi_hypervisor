...
- リンカスクリプトで AT 指定で VMA(仮想アドレス/実行アドレス) と LMA(ロードアドレス)を分けると、バイナリには VMA 基準のデバッグ情報が埋め込まれる
    - よって起動直後の物理アドレスで動いているときはデバッグ情報が正しく参照されない
    - LMA が `0x80000` で LMA が `ffff000000080000` など
    - この場合は gdb で `add-symbol-file` を実行するときにオフセットを指定する
        - `add-symbol-file hv/build/kernel.elf 0x80000`
